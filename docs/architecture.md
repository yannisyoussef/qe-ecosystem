...
| Durable query indexes | Relational indexes over projected runs: one row per run for listings, one per history occurrence for exact history and flakiness, both derived from a replay of the archived source | Rebuildable derived state, never truth; nothing in SQL decides a verdict, an identity, or flakiness; the history key stays (project, runner name, historical id); a whole run is replayed from its own source instead; a project with missing or stale index rows refuses cross-run answers rather than serving part of itself (ADR-0011) |
| Run expiry and retention | An absolute instant supplied at ingestion, stored beside the run, and enforced only by explicit bounded maintenance that deletes expired runs and reclaims bytes no run anywhere references | Expiry is ingestion context like the project, never derived from events or ingestion time, and never moved by re-ingestion; run deletion cascades source lines, blob relations, and the expiry itself, never the global blob catalog; a blob's catalog row is deleted before its bytes, under a guard that no run references the hash, so a failure can only leave an uncatalogued object a later pass collects and never a catalogued hash whose bytes are gone; ingestion and destructive maintenance are serialized by one shared/exclusive advisory lock, held once per mutation; nothing is scheduled (ADR-0010) |
